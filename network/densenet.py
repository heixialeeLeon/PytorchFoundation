...
class DenseNet169(torch.nn.Module):
    def __init__(self, num_classes, fix_layers=0, dropout_p=None, pretrained=False):
        ...
        self.model_name = "DenseNet169"
        pretrained_model = models.densenet169(pretrained=pretrained)
        features = list(pretrained_model.features.children())
        assert fix_layers < len(features)
        for layer in features[:fix_layers+1]:
            for p in layer.parameters():
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(dropout_p) if dropout_p is not None else None
        self.classifier = nn.Linear(in_features=1664, out_features=num_classes, bias=True)
        nn.init.kaiming_normal_(self.classifier.weight)
        nn.init.constant_(self.classifier.bias, 0)
...
class DenseNet201(torch.nn.Module):
    def __init__(self, num_classes, fix_layers=0, dropout_p=None, pretrained=False):
        ...
        pretrained_model = models.densenet201(pretrained=pretrained)
        features = list(pretrained_model.features.children())
        assert fix_layers < len(features)
        for layer in features[:fix_layers+1]:
            for p in layer.parameters():
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(dropout_p) if dropout_p is not None else None
        self.classifier = nn.Linear(in_features=1920, out_features=num_classes, bias=True)
        nn.init.kaiming_normal_(self.classifier.weight)
        nn.init.constant_(self.classifier.bias, 0)
...
class DenseNet161(torch.nn.Module):
    def __init__(self, num_classes, fix_layers=0, dropout_p=None, pretrained=False):
        ...
        pretrained_model = models.densenet161(pretrained=pretrained)
        features = list(pretrained_model.features.children())
        assert fix_layers < len(features)
        for layer in features[:fix_layers+1]:
            for p in layer.parameters():
        ...
        self.avg_pool = nn.AdaptiveAvgPool2d((1, 1))
        self.dropout = nn.Dropout(dropout_p) if dropout_p is not None else None
        self.classifier = nn.Linear(in_features=2208, out_features=num_classes, bias=True)
        nn.init.kaiming_normal_(self.classifier.weight)
        nn.init.constant_(self.classifier.bias, 0)
